...
- automation endpoint защищен rate limiter-ом
...
flowchart LR
    ...
    B --> G["Infrastructure: JWT / Hashing / Signing"]
```
...
Работает так же, как `registry/search`, но дополнительно ограничен rate limiter-ом.
...
- лимитируется по `x-forwarded-for`
- если его нет, по `x-real-ip`
- если и его нет, используется общий fallback-ключ `global`

Важно:

- rate limiter сейчас in-memory
- в multi-instance deployment это не распределенный limiter
...
- API key / service account модель для ATS вместо JWT пользователя `hr`
- OpenAPI / Swagger
